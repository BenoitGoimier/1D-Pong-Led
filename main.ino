...
#define CTF_SPEED_INCREMENT 5     // Amount of speed increase (decrease in delay) when a point is captured
...
GameMode gameMode = CTF;  // Current game mode
int selectedGame = 0;     // Index of the currently selected game in the choice menu (0: Pong, 1: Reflexe, 2: Push)
...
void choiceLoop() {
  ...
  if (digitalRead(PLAYER1_PIN) == LOW) {
    ...
    selectedGame = (selectedGame + 1) % 4;  // Cycle through 0, 1, 2, 3
  }
...
void pongSetup() {
  ...
  FastLED.clear();
  fill_solid(leds + NUM_LEDS / 2 - 2, 4, ball.color);
  FastLED.show();
  delay(600);
  fill_solid(leds + NUM_LEDS / 2 - 4, 8, ball.color);
  FastLED.show();
  delay(600);
  fill_solid(leds + NUM_LEDS / 2 - 6, 12, ball.color);
  FastLED.show();
  delay(600);
...
void reflexeLoop() {
  ...
  unsigned long delaiReaction = millis() - start;  // Calculate the player's reaction time
...
void reflexSetup() {
  ...
  // Display countdown animation
  fill_solid(leds + NUM_LEDS / 2 - 2, 4, CRGB::Green);
  FastLED.show();
  delay(rand() % 200 + 400);
  fill_solid(leds + NUM_LEDS / 2 - 4, 8, CRGB::Green);
  FastLED.show();
  delay(rand() % 200 + 400);
...
void reflexDefeat() {
  ...
  while (millis() - loopStart < DEFEAT_DELAY) {
    ...
    fill_rainbow_circular(leds + 30, NUM_LEDS / 2, (millis() - loopStart), reversed);
    FastLED.show();
  }
  gameMode = CHOICE;  // Go back to the game selection
}
...
void handlePushGameInput() {
  ...
  if (digitalRead(PLAYER1_PIN) == LOW && !player1.press) {
    player1.press = true;
    ball.x++;  // Move the ball to the right
  }

  // Handle input from Player 2
  if (digitalRead(PLAYER2_PIN) == LOW && !player2.press) {
    player2.press = true;
    ball.x--;  // Move the ball to the left
  }
...
void handleCtfInput() {
  ...
  if (digitalRead(PLAYER1_PIN) == LOW && !player1.press) {
    player1.press = true;
    if (ship1.position == flagPosition) {
      player2.score = max(0, ++player1.score);
      flagCaptured = true;
    } else {
      player1.score = player1.score - 1;  // Prevent score from going below 0
    }
    ship1.speed = CTF_BASE_SPEED - player1.score * CTF_SPEED_INCREMENT; 
  }

  // Handle input from Player 2
  if (digitalRead(PLAYER2_PIN) == LOW && !player2.press) {
    player2.press = true;
    if (ship2.position == flagPosition) {
      player2.score = max(0, ++player2.score);
      flagCaptured = true;
    } else {
      player2.score = player2.score - 1;  // Prevent score from going below 0
    }
    ship2.speed = CTF_BASE_SPEED - player2.score * CTF_SPEED_INCREMENT;
  }
